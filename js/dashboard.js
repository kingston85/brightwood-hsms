...
function renderAdminDashboard() {
  ...
  const pendingPayments = DB.data.paymentSubmissions.filter(p => p.status === 'Pending');
  ...
  document.getElementById('mainContent').innerHTML = `
    ...
    <div class="card p-4 border-l-4 border-l-amber-400 flex items-center justify-between no-print">
      <p class="text-sm"><strong>${pendingPayments.length}</strong> payment submission(s) awaiting verification (Mobile Money / Bank Transfer).</p>
      <button class="btn btn-secondary btn-sm" onclick="navigate('fees'); setTimeout(() => setFeesTab('verification'), 30)">Review Now</button>
    </div>` : ''}
...
function announcementsWidget() {
  const list = (typeof scopedAnnouncements === 'function' ? scopedAnnouncements() : []).slice(0, 4);
  return `
    <div class="card p-5">
      <div class="flex items-center justify-between mb-3">
        <h3 class="font-bold">📢 Latest Announcements</h3>
        <a class="text-xs text-brand-600 hover:underline cursor-pointer no-print" onclick="navigate('announcements')">View all</a>
      </div>
      <div class="space-y-2">
        ${list.map(a => `
          <div class="border-b border-slate-100 last:border-0 pb-2 last:pb-0">
            <div class="font-medium text-sm">${esc(a.title)}</div>
            <div class="text-xs text-slate-400">${esc(a.date)} &middot; ${esc(a.postedByName)}</div>
          </div>
        `).join('') || '<p class="text-sm text-slate-400">No announcements yet.</p>'}
      </div>
    </div>
  `;
}
...
function upcomingEventsWidget() {
  ...
  const list = (typeof scopedEvents === 'function' ? scopedEvents() : [])
  ...
  return `
    <div class="card p-5">
      <div class="flex items-center justify-between mb-3">
        ...
        <a class="text-xs text-brand-600 hover:underline cursor-pointer no-print" onclick="navigate('calendar')">View calendar</a>
      </div>
      <div class="space-y-2">
        ${list.map(e => `
          <div class="flex items-center justify-between border-b border-slate-100 last:border-0 pb-2 last:pb-0">
            <div>
              <div class="font-medium text-sm">${esc(e.title)}</div>
              <div class="text-xs text-slate-400">${esc(e.date)}</div>
            </div>
            <span class="badge badge-${EVENT_TYPES[e.type] || 'slate'}">${esc(e.type)}</span>
          </div>
...
function assignmentsDueWidget(sectionIds) {
  const today = todayISO();
  const list = DB.data.assignments
    .filter(a => sectionIds.includes(a.sectionId) && a.dueDate >= today)
    .sort((a, b) => a.dueDate.localeCompare(b.dueDate))
    .slice(0, 4);
  return `
    <div class="card p-5">
      <div class="flex items-center justify-between mb-3">
        <h3 class="font-bold">🧾 Assignments Due Soon</h3>
        <a class="text-xs text-brand-600 hover:underline cursor-pointer no-print" onclick="navigate('assignments')">View all</a>
      </div>
      <div class="space-y-2">
        ${list.map(a => `
          <div class="flex items-center justify-between border-b border-slate-100 last:border-0 pb-2 last:pb-0">
            <div>
              <div class="font-medium text-sm">${esc(a.title)}</div>
              <div class="text-xs text-slate-400">${DB.subjectName(a.subjectId)} &middot; ${DB.classSectionLabel(a.classId, a.sectionId)}</div>
            </div>
            <span class="text-xs font-semibold text-slate-500">${esc(a.dueDate)}</span>
          </div>
        `).join('') || '<p class="text-sm text-slate-400">Nothing due soon.</p>'}
      </div>
    </div>
  `;
}
...
function renderStudentDashboard() {
  ...
  document.getElementById('mainContent').innerHTML = `
    ...
    <div class="grid lg:grid-cols-3 gap-4">
      ${announcementsWidget()}
      ${assignmentsDueWidget([stu.sectionId])}
      ${upcomingEventsWidget()}
    </div>
  `;
}
